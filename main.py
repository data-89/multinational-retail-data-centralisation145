...
list_tables = db_connector.list_db_tables() #['legacy_store_details', 'legacy_users', 'orders_table']

data_extractor = DataExtractor(db_connector)
result_dataframes = data_extractor.read_rds_table(list_tables)
legacy_store_details_df = data_extractor.read_rds_table(list_tables[0])
legacy_users_df = data_extractor.read_rds_table(list_tables[1])
print(f"Legacy users dtypes: {legacy_users_df.dtypes}")
print(f"Legacy users info: {legacy_users_df.info()}")
print(f"Legacy store info: {legacy_store_details_df}")
...
number_of_stores = data_extractor.list_number_of_stores(header, return_number_url)  # {'statusCode': 200, 'number_stores': 451}
stores_data = data_extractor.retrieve_stores_data(header, retrieve_store_url)

data_cleaner = DataCleaning()
cleaned_user_data = data_cleaner.clean_user_data(legacy_users_df)
...
datetime_df_cleaned = data_cleaner.clean_datetime_data(date_details_df)
